...
class Simulator {
  ...
  constructor(epochs: number, rows: number, cols: number) {
    this.epochs = epochs;
    this.rows = rows;
    this.cols = cols;
  ...
  private computeNextH_x(k: number, i: number, j: number) {
    ...
    // Perfect matching layer has to be implemented here
    const x_curl =
  ...
  private computeNextE_zx(k: number, i: number, j: number) {
    ...
    const z_curl =
      (this.Hy[this.idx_Hy(k, i, j)] - this.Hy[this.idx_Hy(k, i - 1, j)]) /
      this.dx;
  ...
  private computeNextE_zy(k: number, i: number, j: number) {
    ...
    const z_curl =
      (this.Hx[this.idx_Hx(k, i, j)] - this.Hx[this.idx_Hx(k, i, j - 1)]) /
      this.dy;
  ...
  public buildCoefficientArrays(
    ...
    const sigma_max_left =
      (-((m + 1) * EPS0 * C) / (2 * this.dx * top)) * Math.log(R);
    const sigma_max_right =
      (-((m + 1) * EPS0 * C) / (2 * this.dx * bottom)) * Math.log(R);
    const sigma_max_up =
      (-((m + 1) * EPS0 * C) / (2 * this.dy * left)) * Math.log(R);
    const sigma_max_down =
      (-((m + 1) * EPS0 * C) / (2 * this.dy * right)) * Math.log(R);

    for (let j = 0; j < top; j++) {
      const s = Math.pow((top - (j + 1)) / top, m);
    ...
    for (let j = this.cols + 2 - bottom; j < this.cols + 2; j++) {
      const s = Math.pow((j + 1 - (this.cols + 2 - bottom)) / bottom, m);
    ...
    for (let i = 0; i < left; i++) {
      const s = Math.pow((left - (i + 1)) / left, m);
    ...
    for (let i = this.rows + 2 - right; i < this.rows + 2; i++) {
      const s = Math.pow((i + 1 - (this.rows + 2 - right)) / right, m);
  ...
  private idx_Ez(t: number, i: number, j: number) {
  ...
  private idx_cEz(i: number, j: number) {
...
const epochs = 500;
const simulator = new Simulator(epochs, 400, 400);
simulator.injectGaussSineSource(100, 100);
simulator.injectRectMaterial(150, 80, 40, 40, 1.0, 2.8);
simulator.injectRectPEC(300, 80, 40, 40);
simulator.buildCoefficientArrays(0, 0, 40, 0);
